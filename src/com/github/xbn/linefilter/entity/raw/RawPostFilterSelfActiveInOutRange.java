...
/**
   <P>A {@code RawEntityOnOffFilter} that is dependant on the <I>being filtered</I> entity's {@linkplain RawEntity#getFullyActiveCount() fully-active count} being in a range.</P>

   @since 0.1.0
   @author  Copyright (C) 2014, Jeff Epstein ({@code aliteralmind __DASH__ github __AT__ yahoo __DOT__ com}), dual-licensed under the LGPL (version 3.0 or later) or the ASL (version 2.0). See source code for details. <A HREF="http://xbnjava.aliteralmind.com">{@code http://xbnjava.aliteralmind.com}</A>, <A HREF="https://github.com/aliteralmind/xbnjava">{@code https://github.com/aliteralmind/xbnjava}</A>
 **/
public class RawPostFilterSelfActiveInOutRange<O,L extends RawLine<O>> extends AbstractRawPostFilterActiveInOutRange<O,L>  {
   /**
    ...
    **/
   public OnOffAbort getPostState(RawEntity<O,L> entity_beingFiltered, RawLine<O> ignored2, O ignored3)  {
      return  getPostStateForCount(entity_beingFiltered);
   }
   public RawPostFilterSelfActiveInOutRange<O,L> getObjectCopy()  {
      return  this;
   }
}
